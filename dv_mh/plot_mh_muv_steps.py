...
def get_p_stellar_mass(mh, params):
    mh = 10**mh
    mstar_range = np.linspace(3, 12, 100)
    alpha_1, f_star10, m_pivot, sigma_star = params
...
def get_p_muv(sfr, stellar_mass, redshift=9.0):
    ...
    muv_mean += auv
    # plt.pcolormesh(np.log10(sfr), np.log10(stellar_mass), muv_mean.T, cmap='hot')
    # plt.colorbar(label=r'${\rm M}_{\rm UV}$')
    # plt.show()
    # quit()
    sigma_kuv = 0.245
...
stellar_params = [0.4709, 0.00988, 2.8e11, 0.2393]
sfr_params = [0.1676, 0.09, -0.09]
# stellar_params = [0.4709, 0.05, 2.8e11, 0.2393]
# sfr_params = [0.009, 0.09, -0.09]
...
if plot_distributions:
    fig, axs = plt.subplots(2, 3, figsize=(8,10), constrained_layout=True)
...
p_mstar = trapezoid(p_mh[:, np.newaxis]*p_mstar.T, x=mh_range, axis=1)  # comoving Mpc^-3 dex^-2

axs[1,1].contourf(mstar_range, sfr_range, p_mstar[:, np.newaxis]*p_sfr.T, levels=50,
                cmap='hot')
...
p_sfr = trapezoid(p_mstar[:, np.newaxis]*p_sfr.T, x=mstar_range, axis=1)  # comoving Mpc^-3 dex^-2

axs[1,2].contourf(sfr_range, muv_range, (p_sfr[:, np.newaxis, np.newaxis]*p_mstar[np.newaxis, :, np.newaxis]*p_muv).sum(axis=1).T, levels=50,
                cmap='hot')
...
p_mstar_muv = trapezoid(p_sfr[:, np.newaxis, np.newaxis]*p_muv, x=sfr_range, axis=0)  # comoving Mpc^-3 dex^-2
p_mstar_muv[np.isnan(p_mstar_muv)] = 0
p_muv = trapezoid(p_mstar[:, np.newaxis]*p_mstar_muv, x=mstar_range, axis=0)  # comoving Mpc^-3 dex^-1
# p_muv = trapezoid(p_mstar_muv, x=mstar_range, axis=0)[::-1]  # comoving Mpc^-3 dex^-1
# muv_range = muv_range[p_muv>0]
# p_muv = p_muv[p_muv>0]
...
axs[0].set_ylabel(r'$\phi(M_{\rm h})\,{\rm [cMpc^{-3}]}$', fontsize=font_size)
axs[0].set_yscale('log')
axs[1].plot(mstar_range, p_mstar, color='cyan')
axs[1].set_xlabel(r'$\log_{10}(M_{\star}/M_\odot)$', fontsize=font_size)
axs[1].set_ylabel(r'$\phi(M_{\star})\,{\rm [cMpc^{-3}]}$', fontsize=font_size)
axs[1].set_yscale('log')
axs[2].plot(sfr_range, p_sfr, color='cyan')
axs[2].set_xlabel(r'$\log_{10}({\rm SFR}/M_\odot {\rm yr}^{-1})$', fontsize=font_size)
axs[2].set_ylabel(r'$\phi({\rm SFR})\,{\rm [cMpc^{-3}]}$', fontsize=font_size)
axs[2].set_yscale('log')
axs[3].plot(muv_range, p_muv, color='cyan')
axs[3].plot(muv_range, p_muv_ref, color='orange', linestyle='dashed')
axs[3].set_xlabel(r'${\rm M}_{\rm UV}$', fontsize=font_size)
axs[3].set_ylabel(r'$\phi({\rm M}_{\rm UV})\,{\rm [cMpc^{-3}]}$', fontsize=font_size)
